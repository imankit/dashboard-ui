...
export const Progressbar = (props) => (
    <div className="progress_bar" onClick={() => props.onProjectClick()}>
        ...
        <div className="storagehead">
            <div className="storage_bar" style={{
                width: props.storageUsed + '%'
            }}></div>
        </div>
    </div>
);
...
const mapStateToProps = (state, selfProps) => {
    ...
    return {
        apiUsed: Math.ceil(apiUsed
            ? (apiUsed / numMaxAPI) * 100
            : 0),
        storageUsed: Math.ceil(storageUsed
            ? (storageUsed / numMaxStorage) / 100
            : 0),
        maxAPI,
        maxStorage
    }
